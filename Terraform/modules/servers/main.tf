...
resource "local_file" "etcd-hosts" {
  content  = templatefile("${path.module}/etchost.tpl",
    {
      master_0_private_ips = hcloud_server.master.0.network[*].ip
      master_1_private_ips = hcloud_server.master.1.network[*].ip
      master_2_private_ips = hcloud_server.master.2.network[*].ip
      haproxy_private_ips = hcloud_server.haproxy.network[*].ip
